...
export default function TableTabs({
	...
	const [tabs, setTabs] = useMarketStore((state) => [
		state.tabs,
		state.setTabs,
	]);
	...
	return (
		...
		>
			<div className="flex flex-col lg:flex-row items-start justify-between lg:border-b lg:border-divider-secondary">
				<TabsList className="space-x-5 lg:space-x-6 border-b border-divider-secondary lg:border-none w-full justify-start rounded-none p-0">
					{listTab.map((tab) => (
						<TabsTrigger
							className={cn(
								"text-sm font-normal font-determination lg:pt-1.5 pb-3 lg:pb-4 flex items-center gap-x-2 border-b-[2px] border-solid pt-0",
								{
				...
				{filterTable && (
					<div className="py-4 lg:py-0 lg:w-max w-full">{filterTable}</div>
				)}
			</div>
