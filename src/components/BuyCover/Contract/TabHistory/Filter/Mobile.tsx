...
const MainMenu = ({ selectedPair, handleChangeStep, openTime, expireTime, handleApply, handleResetFilter, status }: MainMenuProps) => {
    ...
    return <>
                ...
                >
                    <section className="w-full flex items-center justify-between gap-2">
                        <div className="!text-body-14">
                            {
                                selectedPair ? <div className="flex items-center gap-2">
                                    <Image
                                        className="size-5"
                                        ...
                                        alt="logo"
                                    />
                                    <span className="text-typo-primary">{selectedPair?.asset}</span>
                                    <span className="text-typo-secondary"> / USDT</span>
                                </div> : <span className="text-typo-secondary">Select asset</span>
                            }
                        </div>
                        <ChevronIcon />
                    </section>
            ...
            <section>
                ...
                >
                    <section className="w-full flex items-center justify-between gap-2">
                        <div className="!text-body-14">
                            {
                                status ? <p>{STATE_INSURANCES[status]}</p> : <span className="text-typo-secondary">Select status</span>
                            }
                        </div>
                        <ChevronIcon />
                    </section>
        ...
        <Button size="lg" variant="primary" className="w-full justify-center mt-8" onClick={handleApply}>
            Apply
        </Button>
        <Button size="lg" variant="outline" className="w-full justify-center mt-4" onClick={handleResetFilter}>
            Reset filter
        </Button>
    </>;
};
...
const Mobile = ({ isOpen, handleOpenStatusChange, handleChangeAsset, handleChangeOpenTime, handleChangeExpireTime, handleChangeStatus }: FilterMobileProps) => {
    ...
    return (
        <DrawerWrapper
            ...
            prefix={
                step !== STEP_FILTER.BEGIN && <Button size="md" onClick={() => handleChangeStep(STEP_FILTER.BEGIN)}>
            ...
            content={
                <>
                    {
                        step === STEP_FILTER.BEGIN && <MainMenu selectedPair={selectedPair} handleChangeStep={handleChangeStep} openTime={openTime} expireTime={expireTime} status={status} handleApply={handleApply} handleResetFilter={handleResetFilter} />
                    }
                    {
                        step === STEP_FILTER.ASSET && <AssetDrawer selectedPair={selectedPair} handleSelectedPair={handleSelectedPair} />
                    }
                    {
                        step === STEP_FILTER.STATUS && <StatusDropdown status={status} handleSetStatus={(status: string | undefined) => setStatus(status)} />
                    }
                    {
                        step === STEP_FILTER.TIME_START && <CalendarDrawer onChange={setOpenTime} range={openTime} title="T-Start" />
                    }
                    {
                        step === STEP_FILTER.TIME_EXPIRE && <CalendarDrawer onChange={setExpireTime} range={expireTime} title="T-Expire" />
                    }
                </>
