...
const MobileRecord = ({ data, onShowDetail }: RecordProps) => {
    const { token, side, expiredAt, q_claim, margin, q_covered, state, txhash } = useMemo(() => data, [data]);
    const { variant, title } = STATUS_DEFINITIONS[state];
    return (
        <section onClick={() => onShowDetail(data)} className="border border-divider-secondary p-3 rounded">
            ...
            <section className="flex flex-col gap-3">
                <div className="flex items-center justify-between text-body-12">
                    <TooltipCustom
                        content={<p>State</p>}
                        title={<p className="text-typo-secondary border-b border-dashed border-divider-secondary">State</p>}
                        showArrow={true}
                    />
                    <Tag
                        variant={variant}
                ...
                <div className="flex items-center justify-between text-body-12">
                    <p className="text-typo-secondary border-b border-dashed border-divider-secondary">PnL</p>
                    <PnlWrapper
                        margin={margin}
                        q_claim={q_claim}
