...
const ModalInfoFriend = ({ open, handleClose }: TProps) => {
	...
	return (
		<Modal title={""} onRequestClose={handleClose} isOpen={open} modal>
			<div className="flex flex-col gap-y-5">
				<div className="flex flex-col gap-y-4 items-center">
					<img src="/assets/images/icons/user_icon.png" className="w-20 h-20" />
					{dataFriend?.username ? (
				...
				<div className="p-4 w-full flex flex-col gap-y-3 items-center bg-support-black rounded-md">
					<div className="flex items-center justify-between w-full">
						...
						<p className="text-typo-primary">3,000,000 USDT</p>
					</div>
					<div className="flex items-center justify-between w-full">
						<p className="text-typo-secondary">Total contracts</p>
						<p className="text-typo-primary">2,000</p>
					</div>
				</div>
					...
					<div className="p-4 w-full flex flex-col gap-y-3 items-center bg-support-black rounded-md">
						<div className="flex items-center justify-between w-full">
							...
							<p className="text-typo-primary">
								{dataFriend?.totalCommission} USDT
							</p>
						</div>
						{dataFriend?.metadata?.friendType === 1 ? (
							<div className="flex items-center justify-between w-full">
								...
								<p className="text-typo-primary">
									{(dataFriend?.totalCommission as number) * sharePercent} USDT
								</p>
							</div>
						) : null}
					</div>
					...
					<div className="p-4 w-full flex flex-col gap-y-3 items-center bg-support-black rounded-md">
						<div className="flex items-center justify-between w-full">
							...
							<Copy
								...
								prefix={
									<div className="text-typo-accent">{dataFriend?.refCode}</div>
								}
							/>
						</div>
						{dataFriend?.metadata?.friendType === 1 ? (
							<>
								<div className="flex items-center justify-between w-full">
									...
									<p className="text-typo-primary">
										{Number(dataFriend?.metadata?.referralCode?.myPercent) *
											100}{" "}
										% / {sharePercent * 100} %
									</p>
								</div>
