	...
	({ className, showTitle = false }: IOnchainActivity) => {
		...
		const renderContent = (data: TDataTransaction[]) => {
			return data.map((item: TDataTransaction, idx: number) => (
				<div
					className="mx-3 min-w-[320px] border hover:border-divider-primary hover:bg-background-secondary border-divider-secondary rounded-[4px] cursor-pointer whitespace-nowrap bg-white px-3 py-3 shadow sm:px-4"
					key={idx}
				>
