			...
			<div className="mt-16">
				{isMobile ? (
					<PoolProcessMobile
						...
						marginAddress={`${VICTION_SCAN}/address/${INSURANCE_ADDRESS}`}
						claimAddress={`${VICTION_SCAN}/address/${INSURANCE_ADDRESS}`}
						hakifiAddress={`${VICTION_SCAN}/address/${INSURANCE_ADDRESS}`}
						sciAddress={`${VICTION_SCAN}/address/${SCILABS_ADDRESS}`}
					/>
				) : (
					<PoolProcessDesktop
						marginPool={formatNumber(dataPool.marginPool)}
