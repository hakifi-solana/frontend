...
const WhyChoose = () => {
  const reasons = [
    ...
    },
    {
      ...
      label: "Zero Fee",
      desc:"There are no extra charges incurred from the time the contract opens until its expiration.",
    },
    {
      icon: "/assets/images/home/ic_security.png",
      label:"Inclusivity",
      desc: "Utilize Cover-to-Earn with CoFu to manage spot asset risk, derivative positions, and to earn returns.",
    },
  ];
      ...
      <div className="">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-y-10 gap-x-4 sm:gap-6">
          {reasons.map((reason, i) => (
            <div
              key={i}
              className="flex flex-col border border-t-8 rounded-[4px] border-background-primary lg:space-y-6 space-y-3 sm:px-4 lg:p-5 p-4 box-shadow box-radius bg-white"
            >
