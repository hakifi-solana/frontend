	...
	return (
		<div className="rounded-md border-l-8 border-r-8 border-divider-primary max-w-desktop grid w-full grid-cols-2 items-center gap-y-4 bg-background-secondary px-2 lg:py-8 py-4 shadow-md lg:flex lg:px-0 border">
			{titles.map((item) => {
				return (
					<div
						className="col-span-1 flex w-full flex-1 h-max flex-col items-center justify-center gap-y-2"
						key={item.value}
					>
						<div className="lg:text-body-16 text-sm text-typo-primary">{item.title}</div>
						<div className="display-2-desktop text-typo-accent lg:text-[40px] text-3xl font-determination tracking-[2.4px]">
							{item?.prefix || null}
